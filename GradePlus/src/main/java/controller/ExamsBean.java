...
public class ExamsBean extends AbstractBean implements Serializable {
     ...
     * {@link Exam}-Objekte übernimmt.
     */
    private final ExamDAO examDao;

    /**
     * Wird benötigt, um in {@link #notify(User, String)} ein {@link MailBean}-
     * Objekt erzeugen zu können.
     */
     ...
     */
    @Inject
    public ExamsBean(final Session pSession,
     ...
     * {@link #examsOfStudent} und {@link #examsOfExaminer}, sodass
     * {@link #exam} eine zu erstellende Prüfung repräsentiert,
     * {@link #allExams} alle innerhalb des Systems bekannten Prüfungen,
     * {@link #examsOfStudent} alle Prüfungen eines Studenten und
    ...
    public void init() {
        ...
        examsOfStudent = assertNotNull(examDao.getExamsForExaminee(getSession().getUser()));
        examsOfExaminer = assertNotNull(examDao.getExamsForExaminee(getSession().getUser()));
    }
    ...
    /**
     * Gibt die Liste aller innerhalb des Systems bekannten Prüfungstermine
     * zurück.
     *
    ...
    /**
     * Fügt die aktuell angezeigte Prüfung der Liste aller innerhalb der
     * Applikation bekannten Prüfungen hinzu.
     *
     * @return "lectureinstance.xhtml", um auf das Facelet der Übersicht der
     * Prüfungen zu leiten.
     */
    public String save() {
        try {
    ...
    /**
     * Aktualisiert die aktuell angezeigte Prüfung in der Liste aller innerhalb
     * der Applikation bekannten Prüfungen.
     *
     * @return "lectureinstance.xhtml", um auf das Facelet der Übersicht der
     * Prüfungen zu leiten.
     */
    public String update() {
        try {
            ...
            String message = String.format(
                    "Die Daten des Prüfungstermins für %s am %s um %s Uhr, wurden angepasst.\n\nNeue Daten:\nDatum: %s\nUhrzeit: %s Uhr\nDauer: %i Minuten",
                    oldExam.getIlv().getLecture().getName(),
                    oldExam.getDate().toString(),
                    oldExam.getTime().toString(),
    ...
    /**
     * Entfernt die übergebene Prüfung aus der Liste aller bekannten Prüfungen
     * unter Verwendung des entsprechenden Data-Access-Objekts. Sollte die zu
     * entfernende Prüfung nicht in der Liste der Prüfungen vorhanden sein,
     * passiert nichts.
     *
     * @param pExam
    ...
    public String remove(final Exam pExam) {
        List<User> students = assertNotNull(pExam).getStudents();
        examDao.remove(pExam);
        String message = String.format(
                "Der Prüfungstermin für %s am %s um %s Uhr wurde gelöscht. Bitte melden Sie sich bei Bedarf zu einem neuen Termin an.",
                pExam.getIlv().getLecture().getName(),
                pExam.getDate().toString(),
                pExam.getTime().toString()
        );
        for (int i = 0; i < students.size(); i++) {
            //notify(students.get(i), message); TODO !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        }
    ...
    /**
     * Sendet eine Nachricht an den Prüfling, um ihm über Änderungen eines
     * Prüfungstermins zu informieren.
     *
     ...
     *
     * @param instanceLecture Die ILV, der die Prüfung angehören.
     * @param lowerBound Der frühste Zeitpunkt einer Prüfung.
     * @param upperBound Der späteste Zeitpunkt, zu der eine Prüfung noch laufen
     *                   kann. Keine Prüfung darf nach diesem Zeitpunkt beendet
     *                   werden.
     * @param lengthOfExams Die Länge einer Prüfung.
     * @param lengthOfBreaksBetweenExams Die Länge der Pausen zwischen
     *                                   Prüfungen.
     * @param location Der Ort der Prüfungen.
     * @param type Die Art der Prüfungen (z.B. Fachgespräche)
     * @param isGroupExam Gibt an, ob es sich um eine Gruppenprüfung handelt.
     * @param examRegulations Die Prüfungsordnung der Prüfung.
     *
    ...
    public List<Exam> createExamsForTimePeriod(final InstanceLecture instanceLecture,
                                         ...
                                         final int lengthOfExams,
                                         final int lengthOfBreaksBetweenExams,
                                         final String location,
                                         final String type,
                                         final boolean isGroupExam,
        ...
        while (endOfExam.compareTo(upperBound) <= 0) {
            ...
            startOfExam = startOfExam.plusMinutes(lengthOfExams+lengthOfBreaksBetweenExams);
     ...
     *
     * @return {@code true} Falls im gegebenen Zeitraum bereits eine Prüfung
     * stattfindet, sonst {@code false}.
     */
    private boolean isTimeSlotEmpty(final LocalDateTime lowerBound, final LocalDateTime upperBound) {

        for (Exam currExam : examsOfExaminer) {

            LocalDateTime currLowerBound = LocalDateTime.of(currExam.getDate(), currExam.getTime());
            LocalDateTime currUpperBound = currLowerBound.plusMinutes(currExam.getExamLength());

            if ((lowerBound.isBefore(currLowerBound) && upperBound.isAfter(currLowerBound))
                || (currLowerBound.isBefore(lowerBound) && currUpperBound.isAfter(lowerBound))
                || lowerBound.equals(currLowerBound)) {
                return true;
            }
        }
        return false;
